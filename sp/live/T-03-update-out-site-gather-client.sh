...
# _enter_ok_go_on
echo ansible $out_gc_list -m synchronize -a \'src=/var/sp/apps/gather-client/ dest=/var/sp/apps/gc-T/  rsync_opts="--exclude=gather-conf.properties"\'
     ansible $out_gc_list -m synchronize -a  'src=/var/sp/apps/gather-client/ dest=/var/sp/apps/gc-T/  rsync_opts="--exclude=gather-conf.properties"' >/dev/null

_h2 "[ Step ${ITEM} ]: 重启外围gather-client... "; let ITEM+=1
echo ansible $out_gc_list -m shell -a \'docker restart sp-gc-T\'
     ansible $out_gc_list -m shell -a  'docker restart sp-gc-T'
